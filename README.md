<img src="hellworm.png" alt="Hellworm Hunter logo — a molten, coiled hellworm" width="140" align="right">

# Hellworm Hunter

Landing page for **Hellworm Hunter** — a lightweight Overwolf overlay for
Diablo IV Helltides: a compass arrow to the nearest Hellworm spawn, a live
zone map of community-sourced spawn locations, and a route planner.

Built exclusively on Overwolf's official Diablo IV game events (player
location). No memory reading, no automation, no game-file access.

- **Status:** in development — Overwolf app proposal under review
- **Contact:** ceruleanparadigms@gmail.com

This repository hosts the public landing page only. Application source is not
published here.
